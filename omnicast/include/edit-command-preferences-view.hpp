...
class EditCommandPreferencesView : public View {
  ...
  std::vector<PreferenceField *> m_preferences;

  void handleSubmit() { qDebug() << "Submitting preferences"; }
...
public:
  ...
  EditCommandPreferencesView(AppWindow &app, const std::shared_ptr<AbstractCmd> &command)
    ...
    m_preferences.reserve(preferences.size());
