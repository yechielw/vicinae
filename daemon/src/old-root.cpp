...
using RootItem = std::variant<AppItem>;

class RootListItemDelegate : public AbstractNativeListItemDelegate {
  QWidget *createItem(const QVariant &data, int role) override {
    if (role == RootItemRole::App) {
      auto app = data.value<AppItem>();

      return new ListItemWidget(ImageViewer::createFromModel(ThemeIconModel{
                                    .iconName = app.entry->iconName()}),
                                app.entry->name, "", "Application");
    }

    if (role == RootItemRole::ExtensionCommand) {
    }
...
class RootListModel : public AbstractNativeListModel {
  QList<RootItem> items;

public:
  void addItem(const RootItem &item) {
    if (auto app = std::get_if<AppItem>(&item)) {
      AbstractNativeListModel::addItem(QVariant::fromValue(*app),
                                       RootItemRole::App);
    }
  }
};
