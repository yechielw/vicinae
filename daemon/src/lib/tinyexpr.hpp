...
using te_fun6 = te_type (*)(te_type, te_type, te_type, te_type, te_type,
                            te_type);
using te_fun7 = te_type (*)(te_type, te_type, te_type, te_type, te_type,
                            te_type, te_type);
...
using te_confun4 = te_type (*)(const te_expr*, te_type, te_type, te_type,
                               te_type);
using te_confun5 = te_type (*)(const te_expr*, te_type, te_type, te_type,
                               te_type, te_type);
using te_confun6 = te_type (*)(const te_expr*, te_type, te_type, te_type,
                               te_type, te_type, te_type);
using te_confun7 = te_type (*)(const te_expr*, te_type, te_type, te_type,
                               te_type, te_type, te_type, te_type);
...
using te_variant_type =
    std::variant<te_type, const te_type*,  // indices 0-1
                                           // indices 2-9
                 te_fun0, te_fun1, te_fun2, te_fun3, te_fun4, te_fun5, te_fun6,
                 te_fun7,
                 // indices 10-17
                 te_confun0, te_confun1, te_confun2, te_confun3, te_confun4,
                 te_confun5, te_confun6, te_confun7>;
...
class te_string_less {
   public:
    [[nodiscard]] bool operator()(const std::string& lhv,
...
class te_parser {
   public:
    ...
    /// @private
    [[nodiscard]] te_type get_result() const volatile noexcept {
        return m_result;
    }
    ...
    [[nodiscard]] bool success() const volatile noexcept {
    ...
    /// @private
    [[nodiscard]] int64_t get_last_error_position() const volatile noexcept {
        return m_errorPos;
    }

    /// @returns Any error message from the last parse.
    [[nodiscard]] const std::string& get_last_error_message() const noexcept {
    ...
    void remove_variable_or_function(te_variable::name_type var) {
        auto foundVar = m_customFuncsAndVars.find(te_variable{
    ...
    /// @private
    [[nodiscard]] const std::set<te_variable>& get_variables_and_functions()
        const noexcept {
        return m_customFuncsAndVars;
    }

    /// @returns The list of custom variables and functions.
    [[nodiscard]] std::set<te_variable>&
    get_variables_and_functions() noexcept {
        return m_customFuncsAndVars;
    }
    ...
    /// @private
    [[nodiscard]] char get_decimal_separator() const volatile noexcept {
        return m_decimalSeparator;
    }
    ...
    /// @private
    [[nodiscard]] char get_list_separator() const volatile noexcept {
        return m_listSeparator;
    }
    ...
    void set_list_separator(const char sep) {
        if (sep != ',' && sep != ';') {
            throw std::runtime_error(
                "List separator must be either a ',' or ';'.");
        }
        m_listSeparator = sep;
    }
    ...
    [[nodiscard]] bool is_function_used(const std::string_view name) const {
        return m_usedFunctions.find(te_variable::name_type{name}) !=
               m_usedFunctions.cend();
    }
    ...
    [[nodiscard]] bool is_variable_used(const std::string_view name) const {
        return m_usedVars.find(te_variable::name_type{name}) !=
               m_usedVars.cend();
    }
#endif
    ...
    [[nodiscard]] const std::string& get_expression() const noexcept {
        return m_expression;
    }
   ...
   private:
    ...
    /// @private
    [[nodiscard]] const te_expr* get_compiled_expression() const
        volatile noexcept {
        return m_compiledExpression;
    }
    ...
    void validate_name(const te_variable& var) const {
        ...
        if (varCharPos != var.m_name.cend()) {
            throw std::runtime_error(
                std::string("Invalid character in variable name: ") +
                var.m_name);
        }
    }
    ...
    [[nodiscard]] constexpr static bool is_name_char_valid(
    ...
    [[nodiscard]] std::set<te_variable>::iterator find_variable_or_function(
        ...
        return m_customFuncsAndVars.find(
            te_variable{te_variable::name_type{name}, static_cast<te_type>(0.0),
                        TE_DEFAULT, nullptr});
    }
    ...
    [[nodiscard]] constexpr static auto is_variadic(
        const te_variable_flags type) {
        return (((type)&TE_VARIADIC) != 0);
    }

    /// @returns Number of parameters that a function/variable takes.
    [[nodiscard]] inline static auto get_arity(
        const te_variant_type& var) noexcept {
    ...
    [[nodiscard]] constexpr static bool is_constant(
        const te_variant_type& var) noexcept {
        return var.index() == 0;
    }

    [[nodiscard]] constexpr static te_type get_constant(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_type>(var));
        return std::get<0>(var);
    }

    [[nodiscard]] constexpr static bool is_variable(
        const te_variant_type& var) noexcept {
        return var.index() == 1;
    }

    [[nodiscard]] constexpr static const te_type* get_variable(
        const te_variant_type& var) {
        assert(std::holds_alternative<const te_type*>(var));
        return std::get<1>(var);
    }

    [[nodiscard]] constexpr static bool is_function(
        const te_variant_type& var) noexcept {
        return (var.index() >= 2 && var.index() <= 9);
    }

    [[nodiscard]] constexpr static bool is_function0(
        const te_variant_type& var) noexcept {
        return var.index() == 2;
    }

    [[nodiscard]] constexpr static te_fun0 get_function0(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun0>(var));
        return std::get<2>(var);
    }

    [[nodiscard]] constexpr static bool is_function1(
        const te_variant_type& var) noexcept {
        return var.index() == 3;
    }

    [[nodiscard]] constexpr static te_fun1 get_function1(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun1>(var));
        return std::get<3>(var);
    }

    [[nodiscard]] constexpr static bool is_function2(
        const te_variant_type& var) noexcept {
        return var.index() == 4;
    }

    [[nodiscard]] constexpr static te_fun2 get_function2(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun2>(var));
        return std::get<4>(var);
    }

    [[nodiscard]] constexpr static bool is_function3(
        const te_variant_type& var) noexcept {
        return var.index() == 5;
    }

    [[nodiscard]] constexpr static te_fun3 get_function3(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun3>(var));
        return std::get<5>(var);
    }

    [[nodiscard]] constexpr static bool is_function4(
        const te_variant_type& var) noexcept {
        return var.index() == 6;
    }

    [[nodiscard]] constexpr static te_fun4 get_function4(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun4>(var));
        return std::get<6>(var);
    }

    [[nodiscard]] constexpr static bool is_function5(
        const te_variant_type& var) noexcept {
        return var.index() == 7;
    }

    [[nodiscard]] constexpr static te_fun5 get_function5(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun5>(var));
        return std::get<7>(var);
    }

    [[nodiscard]] constexpr static bool is_function6(
        const te_variant_type& var) noexcept {
        return var.index() == 8;
    }

    [[nodiscard]] constexpr static te_fun6 get_function6(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun6>(var));
        return std::get<8>(var);
    }

    [[nodiscard]] constexpr static bool is_function7(
        const te_variant_type& var) noexcept {
        return var.index() == 9;
    }

    [[nodiscard]] constexpr static te_fun7 get_function7(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_fun7>(var));
        return std::get<9>(var);
    }

    [[nodiscard]] constexpr static bool is_closure(
        const te_variant_type& var) noexcept {
        return (var.index() >= 10 && var.index() <= 17);
    }

    [[nodiscard]] constexpr static bool is_closure0(
        const te_variant_type& var) noexcept {
        return var.index() == 10;
    }

    [[nodiscard]] constexpr static te_confun0 get_closure0(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun0>(var));
        return std::get<10>(var);
    }

    [[nodiscard]] constexpr static bool is_closure1(
        const te_variant_type& var) noexcept {
        return var.index() == 11;
    }

    [[nodiscard]] constexpr static te_confun1 get_closure1(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun1>(var));
        return std::get<11>(var);
    }

    [[nodiscard]] constexpr static bool is_closure2(
        const te_variant_type& var) noexcept {
        return var.index() == 12;
    }

    [[nodiscard]] constexpr static te_confun2 get_closure2(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun2>(var));
        return std::get<12>(var);
    }

    [[nodiscard]] constexpr static bool is_closure3(
        const te_variant_type& var) noexcept {
        return var.index() == 13;
    }

    [[nodiscard]] constexpr static te_confun3 get_closure3(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun3>(var));
        return std::get<13>(var);
    }

    [[nodiscard]] constexpr static bool is_closure4(
        const te_variant_type& var) noexcept {
        return var.index() == 14;
    }

    [[nodiscard]] constexpr static te_confun4 get_closure4(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun4>(var));
        return std::get<14>(var);
    }

    [[nodiscard]] constexpr static bool is_closure5(
        const te_variant_type& var) noexcept {
        return var.index() == 15;
    }

    [[nodiscard]] constexpr static te_confun5 get_closure5(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun5>(var));
        return std::get<15>(var);
    }

    [[nodiscard]] constexpr static bool is_closure6(
        const te_variant_type& var) noexcept {
        return var.index() == 16;
    }

    [[nodiscard]] constexpr static te_confun6 get_closure6(
        const te_variant_type& var) {
        assert(std::holds_alternative<te_confun6>(var));
        return std::get<16>(var);
    }

    [[nodiscard]] constexpr static bool is_closure7(
        const te_variant_type& var) noexcept {
        return var.index() == 17;
    }

    [[nodiscard]] constexpr static te_confun7 get_closure7(
        const te_variant_type& var) {
    ...
    [[nodiscard]] inline static te_expr* new_expr(
        const te_variable_flags type, te_variant_type value,
        ...
        if (parameters.size()) {
            std::copy(parameters.begin(), parameters.end(),
                      ret->m_parameters.begin());
        }
        return ret;
    }

    [[nodiscard]] inline static te_expr* new_expr(const te_variable_flags type,
    ...
    [[nodiscard]] te_expr* te_compile(const std::string_view expression,
    ...
    [[nodiscard]] static auto find_lookup(state* s,
                                          const std::string_view name) {
